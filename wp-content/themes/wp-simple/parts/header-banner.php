<?php if (is_front_page() && !is_paged())  { ?>
    ...
    <?php } else if (nimbus_get_option('fp-banner-toggle') == '3') { ?>
        <?php // Added to customise front page header ?>
<div style="padding: 26px; background-color: #ea940d;"></div>        
        <?php } else { ?>     
...
<?php } else { ?>
    ...
    if ( has_post_thumbnail() ) {
        ...
        if (!empty($thumb_url)) {
        	...
        	$nimbus_parallax="data-parallax='scroll' data-image-src='" . get_template_directory_uri() . "/assets/images/preview/sunrise.jpg' style='background: rgba(0, 0, 0, 0);'";
           //echo "im here";
